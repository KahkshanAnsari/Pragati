      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          {pilot.status === 'completed' && (
            (() => {
              const rating = id ? getRatingForPilot(id) : null;
              if (rating) {
                return (
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-xs">
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                      Evaluation Completed (★ {rating.overall_rating}.0/5)
                    </span>
                    <Button
                      variant="secondary"
                      onClick={() => navigate(`/government/pilots/${pilot.id}/evaluate`)}
                      className="border-amber-300 bg-amber-50 text-amber-900 hover:bg-amber-100 text-xs font-semibold py-2 px-3 flex items-center gap-1.5"
                    >
              ...
              return (
                <Button
                  onClick={() => navigate(`/government/pilots/${pilot.id}/evaluate`)}
                  className="bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold py-2 px-3.5 flex items-center gap-1.5 shadow-xs"
                >
