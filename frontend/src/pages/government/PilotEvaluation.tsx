...
export const PilotEvaluation: React.FC = () => {
  ...
  const [recommendation, setRecommendation] = useState<'yes' | 'conditional' | 'no'>('yes');
  ...
  const officerName = (profile as any)?.name || 'Rajesh Kumar';
  const officerDesignation = (profile as any)?.designation || 'Joint Commissioner';
  const evalDate = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });
  ...
  const fetchPilotData = async () => {
    ...
    try {
      ...
      if (saved) {
        ...
        setRecommendation(saved.recommendation || 'yes');
        setOfficerComments(saved.feedback || '');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const effectiveFeedback = officerComments.trim() || [
      strengths.trim() ? `Strengths: ${strengths.trim()}` : '',
      improvements.trim() ? `Areas for Improvement: ${improvements.trim()}` : '',
      `Adoption Recommendation: ${recommendation === 'yes' ? 'Recommended for Government Adoption' : recommendation === 'conditional' ? 'Recommended with Conditions' : 'Not Recommended'}`,
    ].filter(Boolean).join('. ');

    if (!effectiveFeedback && !strengths.trim()) {
      toast.error('Please provide officer feedback or strengths before submitting');
      return;
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto font-sans">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* 3. Written Feedback Section */}
        <Card className="p-6 border border-slate-200 space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">
              3. Qualitative Assessment &amp; Written Feedback <span className="text-red-500">*</span>
            </label>
            <p className="text-xs text-slate-500">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                placeholder="Key technical capabilities, sensor reliability, rapid issue resolution, proactive communication..."
              />
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Documentation timelines, field calibration procedures, training manual localization..."
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-navy-900 mb-1.5">
                Would you recommend this startup for government adoption? <span className="text-red-500">*</span>
              </label>
              <div className="flex flex-wrap gap-3">
                {[
                  { id: 'yes', label: 'Yes, unconditionally recommended' },
                  { id: 'conditional', label: 'Yes, with specific operational conditions' },
                  { id: 'no', label: 'No, does not meet procurement criteria' },
                ].map((opt) => (
                  <label
                    key={opt.id}
                    className={`flex items-center gap-2 px-3.5 py-2 rounded-lg border text-xs font-semibold cursor-pointer transition-all ${
            ...
            <div>
              <label className="block text-xs font-bold text-navy-900 mb-1">
                Officer Comments / Final Remarks <span className="text-red-500">*</span>
              </label>
              <Textarea
                rows={3}
                required
                ...
                placeholder="Example: KrishiVision Technologies executed the crop disease detection pilot with high technical rigor in Maharashtra. Multi-spectral imagery delivered 94% verified diagnostic accuracy across all tested agricultural blocks. The team provided prompt technical support. Recommended for statewide scaling under GFR 2017..."
              />
            </div>
          </div>
        </Card>

        {/* 4. Official Officer Attestation */}
        <Card className="p-5 border border-blue-200 bg-blue-50/40 space-y-3">
          <div className="flex items-center justify-between pb-2 border-b border-blue-100 text-xs">
            <div>
              <span className="text-slate-500 block text-[10px] uppercase font-bold">Evaluating Officer</span>
              <strong className="text-navy-900 font-bold">{officerName}</strong>
              <span className="text-slate-500 text-[11px] block">{officerDesignation}</span>
            </div>
            <div className="text-right">
              <span className="text-slate-500 block text-[10px] uppercase font-bold">Evaluation Date</span>
              <span className="text-navy-900 font-semibold">{evalDate}</span>
            </div>
          </div>

          <div className="flex items-start gap-3 text-xs text-blue-950 pt-1">
            <input
              type="checkbox"
              id="attest"
            ...
            />
            <label htmlFor="attest" className="cursor-pointer leading-relaxed font-medium">
              I confirm this evaluation reflects the actual performance of the startup during the pilot and tripartite agreement deliverables.
            </label>
          </div>
        </Card>
