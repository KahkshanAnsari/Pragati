...
import { getRatingForPilot, getRatingsForStartup } from '../../lib/ratingService';
...
> = {
  ...
  0: {
    ...
    pilotCost: '₹10.00 Lakhs (Allocated) • ₹8.40 Lakhs (Utilized)',
    ...
    verificationStatus: 'Government Verified — Officially certified by Municipal Water & Sanitation Department',

    deploymentScope: '25 Municipal Monitoring Locations across Urban Water Grid',
    ...
    return (
      ...
      <div className="space-y-6 max-w-5xl mx-auto font-sans">
        ...
        <div className="space-y-5">
          ...
          {/* Pilot Performance Rating Section */}

          {(() => {

            const pilotRating = activeSolution.pilot_id ? getRatingForPilot(activeSolution.pilot_id) : null;

            const startupRatings = getRatingsForStartup(activeSolution.startup_id || (activeSolution.startup as any)?.id || '');

            const rating = pilotRating || (startupRatings.length > 0 ? startupRatings[0] : null);

            if (rating) {

              return (

                <div className="bg-white rounded-xl border border-amber-200 p-6 shadow-xs space-y-3">

                  <div className="flex items-center justify-between">

                    <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-2">

                      <Star className="w-4 h-4 text-amber-500 fill-amber-500" /> Pilot Performance Rating &amp; Officer Appraisal

                    </h2>

                    <span className="text-sm font-black text-amber-900 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-md">

                      ★ {rating.overall_rating}.0 / 5.0
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-xs pt-1">
                    ...
                    <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                      ...
                      <strong className="text-navy-900 font-bold">★ {rating.technical_performance || 5}/5</strong>

                    </div>

                    <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">

                      <span className="text-[10px] text-slate-400 block">Effectiveness</span>

                      <strong className="text-navy-900 font-bold">★ {rating.solution_effectiveness || 5}/5</strong>

                    </div>

                    <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">

                      <span className="text-[10px] text-slate-400 block">Support</span>

                      <strong className="text-navy-900 font-bold">★ {rating.implementation_support || 5}/5</strong>
                    ...
                    <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                      ...
                      <strong className="text-navy-900 font-bold">★ {rating.timeliness || 5}/5</strong>

                    </div>

                    <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">

                      <span className="text-[10px] text-slate-400 block">KPI Delivery</span>

                      <strong className="text-navy-900 font-bold">★ {rating.kpi_achievement || 5}/5</strong>
                  ...
                  {rating.feedback && (
                    ...
                    <blockquote className="bg-emerald-50/70 border border-emerald-200 rounded-lg p-3 text-xs text-emerald-950 font-medium italic mt-2">
          ...
          <Card key={sol.id} className="p-5 border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-300 transition-all flex flex-col justify-between">
            ...
            <div className="space-y-3">
              ...
              {(() => {
                ...
                const pilotRating = sol.pilot_id ? getRatingForPilot(sol.pilot_id) : null;

                const startupRatings = getRatingsForStartup(sol.startup_id || (sol.startup as any)?.id || '');
                ...
                if (rating) {
                  ...
                  return (
                    ...
                    <div className="bg-amber-50/80 rounded-lg p-2 border border-amber-200 flex items-center justify-between">

                      <div className="flex items-center gap-1.5">

                        <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />

                        <span className="text-xs font-bold text-amber-950">Gov Rating</span>

                      </div>

                      <span className="text-xs font-extrabold text-amber-900">★ {rating.overall_rating}.0/5</span>
