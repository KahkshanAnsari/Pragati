...
const DEFAULT_PILOT_RATINGS: PilotRating[] = [
  {
    id: 'eval-pilot-wrd-001',
    ...
    startup_name: 'AquaSense Technologies',
    department_name: 'Municipal Water & Sanitation Department',
    officer_name: 'Rajesh Kumar',
    officer_designation: 'Joint Commissioner (Water Works)',
    problem_title: 'AI-Powered Smart Water Quality & Pipeline Monitoring System',
    overall_rating: 4.8,
    technical_performance: 5,
    solution_effectiveness: 5,
    implementation_support: 5,
    timeliness: 4,
    kpi_achievement: 5,
    feedback:
      'AquaSense Technologies demonstrated exceptional engineering execution throughout the 90-day pilot. The IoT acoustic and pressure telemetry mesh reduced non-revenue water losses to 18.5% (exceeding the 20% target) and achieved 94% automated anomaly detection accuracy. Field inspections across 25 municipal deployment zones confirmed sensor reliability and zero false alerts. Approved for city-wide municipal procurement under GFR Rule 149.',
    strengths:
      'High precision acoustic leak isolation within 8 meters, automated SCADA valve triggering, seamless edge AI telemetry across 25 stations.',
    improvements:
      'Expansion to secondary municipal feeder lines and additional battery telemetry redundancy.',
    recommendation: 'yes',
    evaluated_at: '2026-08-30T10:00:00.000Z',
  },
];
...
export function getAllRatings(): PilotRating[] {
  try {
    ...
    const parsed: PilotRating[] = raw ? JSON.parse(raw) : [];
    const list = Array.isArray(parsed) ? [...parsed] : [];
    for (const def of DEFAULT_PILOT_RATINGS) {
      if (!list.some((r) => r.pilot_id === def.pilot_id)) {
        list.push(def);
      }
    }
    return list;
